...
$output .= '<div class="row clearfix gs_wpposts">';
    ...
    if ( $GLOBALS['gs_wpposts_loop']->have_posts() ) {
        while ( $GLOBALS['gs_wpposts_loop']->have_posts() ) {
            ...
            $gswppost_wppost_details         = get_the_excerpt();
            $gswppost_wppost_details_link       = get_the_permalink();
            $gswppost_wppost_details            = (strlen($gswppost_wppost_details) > 50) ? substr($gswppost_wppost_details,0, $desc_limit ).'...<a href="'.$gswppost_wppost_details_link.'" target="_self">'.strip_tags( $gswppost_op_wppost_details_readmore ).'</a>' : strip_tags( $gswppost_wppost_details);
...
$output .= '</div>'; // end row
if ( !empty( $pagination ) && 'on' ==  $pagination ) :
